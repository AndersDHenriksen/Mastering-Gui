...
class ImageFileButton(qtw.QPushButton):
    ...
    def on_click(self):
        filename, _ = qtw.QFileDialog.getOpenFileName(None, "Select an image to use", qtc.QDir.homePath(), "Images (*.png *.xpm *.jpg)")
        if filename:
...
class MemeEditForm(qtw.QWidget):
    ...
    def __init__(self):
        ...
        self.bg_padding = qtw.QSpinBox(minimum=0, maximum=100, value=10, valueChanged=self.on_change, suffix=' px')
...
class MainWindow(qtw.QMainWindow):
    ...
    def save_image(self):
        save_file, _ = qtw.QFileDialog.getSaveFileName(None, "Save your image", qtc.QDir.homePath(), "PNG Images (*.png)")
        if save_file:
    ...
    def build_image(self, data):
        ...
        top_block_rect = qtc.QRect(0, 0, self.image.width(), top_px)
